...
int main(int argc, char **argv)
{
  ...
  double compute_average(double *R, int n);
  ...
  FILE *save_file;                 // supersaturation and average radius data
  ...
  double R_avg = compute_average(R,n);

  initialize_file(&save_file,argv[1],"obs_vs_t",p,overlap);

  fprintf(save_file,"%.12e\t%.12e\t%.12e\t%d\n",t,R_avg,chi,n);
